# MSIN0231 — Market Research Assistant

Wikipedia-based market research assistant built with Streamlit and Google Gemini.

## Run locally
pip install -r requirements.txt
streamlit run assignment_1.py

Enter your Google Gemini API key in the sidebar when prompted.

## Test
test_assistant_original.py shows results for an earlier version of the app
test_assistant_improved.py shows results for the current version of the app
